...
public class LaserActivationSystem : NetworkBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (var pair in laserPairs)
        {
            soundPlayers.Add(pair.transform.GetChild(2).GetComponent<SoundPlayer>());
        }
    ...
    private void ManageLasers()
    {
        ...
        if (timer <= 0)
        {
            ...
            {
                soundPlayers[currentPairIndex / 2].CmdPlaySoundForAll("laser_beam");
